...
public class LoopAnimatorScript extends AbstractAnimatorScript implements Controllable, IAnimator {
	public LoopAnimatorScript(IAnimator animator) {
		super(animator);

		if (animator instanceof DelayAnimator) {
			throw new IllegalArgumentException("Putting a DelayAnimator in a LoopAnimatorScript would result " +
			                                   "in an unnecessary endless loop.");
		} else if (animator instanceof LoopAnimatorScript) {
			throw new IllegalArgumentException("Can't put a LoopAnimatorScript in a LoopAnimatorScript.");
		}
	}
